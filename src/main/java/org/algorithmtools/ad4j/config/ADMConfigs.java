...
public class ADMConfigs {
    ...
    public static final ConfigOption<Double> ADM_QUANTILE_IQR_MULTIPLIER = new ConfigOption<>("adm.quantile.iqr.multiplier", 1.5, "Quantile detection IQR multiplier. Support config with sensitivity 0.1s-> low sensitivity; 0.05s-> normal sensitivity; 0.01s-> high sensitivity.");
    ...
    public static final ConfigOption<Double> ADM_ZSCORE_THRESHOLD = new ConfigOption<>("adm.zscore.threshold", 2.0, "Z-score detection threshold.  Support config with sensitivity 0.1s-> low sensitivity; 0.05s-> normal sensitivity; 0.01s-> high sensitivity.");

    //==================ADM GESD Options=================//
    public static final ConfigOption<Boolean> ADM_GESD_USE = new ConfigOption<>("adm.gesd.use", true, "Open GESD detection function.");
    public static final ConfigOption<Double> ADM_GESD_ALPHA = new ConfigOption<>("adm.gesd.alpha", 0.05, "GESD detection alpha. Indicate abnormal significance. Support config with sensitivity 0.1s-> low sensitivity; 0.05s-> normal sensitivity; 0.01s-> high sensitivity.");
